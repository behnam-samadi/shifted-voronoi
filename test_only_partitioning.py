...
#from proposedGrouping.kdtree_grouping_random_selection import *
...
def data_load_proposed_fake_points(data_name, transform):
    if args.data_name == 's3dis':
        data_path = os.path.join(args.data_root, data_name + '.npy')
        data = np.load(data_path)  # xyzrgbl, N*7
        coord, feat, label = data[:, :3], data[:, 3:6], data[:, 6]
...
def test_fake_points(model, criterion, names, test_transform_set):
        ...
        if os.path.isfile(pred_save_path) and os.path.isfile(label_save_path):
            ...
            for aug_id in range(len(test_transform_set)):
                ...
                for i in range(idx_size):
                    idx_part = np.array(idx_data[i])  # ensure numpy array
                ...
                for i in range(batch_num):
                    ...
                    # ⚠️ only keep predictions for real points
    ...
    intersection, union, target = intersectionAndUnion(
        np.concatenate(pred_save), np.concatenate(label_save), args.classes, args.ignore_label
    )
    ...
    logger.info(
        'Val1 result: mIoU/mAcc/allAcc {:.4f}/{:.4f}/{:.4f}.'.format(mIoU1, mAcc1, allAcc1)
    )
...
def save_frames(model, criterion, names, test_transform_set):
    ...
    for idx, item in enumerate(data_list):
        ...
        for aug_id in range(len(test_transform_set)):
            test_transform = test_transform_set[aug_id]
            coord, feat, label, idx_data = data_load_proposed(item, test_transform)
            np.save("/home/samadi/research/tests/results/comparision/visualize_frames/frames/"+str(idx)+"coord.npy", coord)
            np.save("/home/samadi/research/tests/results/comparision/visualize_frames/frames/"+str(idx)+"feat.npy", coord)
