...
class TransitionDown(nn.Module):
    ...
    def forward(self, feats, xyz, offset):
        ...
        offset_temp = torch.cuda.IntTensor([xyz.shape[0] - 10])
        new_offset_temp = torch.cuda.IntTensor([xyz.shape[0] - 10])
        idx2 = pointops.furthestsampling(xyz, offset_temp, new_offset_temp)  # (m)
        first_fps_time += time.time()
        #print('first: ', first_fps_time)
