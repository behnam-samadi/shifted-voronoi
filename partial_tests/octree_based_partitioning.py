...
class OctreeNode:
    def __init__(self, points, indices, depth=0):
        ...
        self.children = []        # up to 8 children
        self.depth = depth
...
def build_octree(points, threshold, depth=0, pbar=None):
    if pbar is not None:
        pbar.update(1)

    indices = torch.arange(points.shape[0], device=points.device)
    return _build_octree(points, indices, threshold, depth, pbar)


def _build_octree(points, indices, threshold, depth=0, pbar=None):
    node = OctreeNode(points, indices, depth)
    ...
    mask = (points > center).int()  # [N, 3] of 0/1
    octant_ids = mask[:, 0] * 4 + mask[:, 1] * 2 + mask[:, 2]  # values in [0..7]

    for octant in range(8):
        oct_mask = (octant_ids == octant)
        if oct_mask.any():
            child_points = points[oct_mask]
            child_indices = indices[oct_mask]
            child_node = _build_octree(child_points, child_indices, threshold, depth + 1, pbar)
            node.children.append(child_node)

    return node


def get_leaf_indices_octree(node):
    leaves = []
...
def collect_leaves_octree(node, leaf_stats):
    if node.is_leaf():
        leaf_stats.append(node.points.shape[0])
    else:
        for child in node.children:
            collect_leaves_octree(child, leaf_stats)


# === chunking functions ===

def proposed_grouping_octree(coord, threshold):
    points = torch.from_numpy(coord).to('cuda')
    rough_max_nodes = 10_000
    with tqdm(total=rough_max_nodes, desc="Building Octree") as pbar:
        octree_root = build_octree(points, threshold, pbar=pbar)

    leaf_indices = get_leaf_indices_octree(octree_root)
    numpy_list = [t.cpu().numpy() for t in leaf_indices]
    return numpy_list


def create_chunks_octree(coord, threshold):
    numpy_list = proposed_grouping_octree(coord, threshold)
...
def create_chunks_return_list_of_lists_octree(coord, threshold):
    points = torch.from_numpy(coord).to('cuda')
    rough_max_nodes = 10_000
    with tqdm(total=rough_max_nodes, desc="Building Octree") as pbar:
        octree_root = build_octree(points, threshold, pbar=pbar)
