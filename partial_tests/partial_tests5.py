...
def build_octree(points, bounds, threshold, depth=0):
    ...
    for box_min, box_max in children_boxes:
        ...
        if child_points.shape[0] > 0:
            child_node = build_octree(child_points, (box_min, box_max), threshold, depth + 1)
...
# Example point cloud
np.random.seed(0)
coord = np.random.rand(10000, 3).astype(np.float32)  # (n, 3)
...
# Build octree
threshold = 65
octree_root = build_octree(points, (min_bound, max_bound), threshold)
