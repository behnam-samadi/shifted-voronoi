...
class KDTreeNode:
    def __init__(self, points, indices, depth=0):
        ...
        # compute variances for info (optional, not needed strictly)
        if points.shape[0] > 1:
            self.variances = points.var(dim=0)
        else:
            self.variances = torch.zeros(points.shape[1], device=points.device)
...
def _build_kdtree(points, indices, threshold, depth=0, pbar=None):
    ...
    # NEW: choose axis by maximum variance instead of depth % 3
    variances = points.var(dim=0)
