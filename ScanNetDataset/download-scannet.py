...
def get_release_scans(release_file):
    print(f"📄 Retrieving scan list from {release_file} ...")
...
def download_file(url, out_file):
    ...
    if os.path.isfile(out_file):
        print(f'✔️  Skipping existing file: {out_file}')
        return
    print(f'⬇️  Downloading {url}')
    try:
        with urllib.request.urlopen(url) as response:
            total = int(response.info().get('Content-Length', -1))
            with tqdm(total=total, unit='B', unit_scale=True, desc=os.path.basename(out_file), ncols=80) as pbar:
                with open(out_file + ".tmp", 'wb') as out_fp:
                    while True:
